            ...
            <form action="update_basket.php" method="POST">
                <?php
                    for ($i = 0; $i <= (int) ($_SESSION['intline2']); $i++) {
                        ...
                        ?>
                    <tr>
                        <td align="right" style="padding-top:20px;"><?= $_SESSION["productid"][$i]; ?></td>
                        ...
                        <td align="center"><input type="number" class="form-control" style="width:75px;" onclick="test(this)" onpaste="return false" ; onchange="if(this.value <= 0) {alert('จำนวนไม่สามารถน้อยกว่า 1'); this.value='1';} else if (this.value > <?= $result['product_amount'] ?>) { alert('กรุณาตรวจสอบจำนวน'); this.value = '<?= $result['product_amount'] ?>'; } else {this.form.submit();}" name="qty_<?= $i ?>" value="<?= $_SESSION["strqty2"][$i]; ?>" /></td>
