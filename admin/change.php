...
?>
           ...
           <?php         } ?>
                            ...
                            <td align="center" label for="textfield"></label>
                                <input type="text" style="width:65px;" value="<?= ($amount_change['change_amount'] ? $amount_change['change_amount'] : "") ?>" class="text-center form-control" <?= ($result_orderlist['od_status'] || ($today >= $strNewDate)) ? "disabled" : "" ?> name="change_amount_<?= $result_orderlist['od_id'] ?>" pattern="^[Z0-9]+$" title="<font size='2' style='padding-left:3px;' color='red'>กรุณาใส่ตัวเลข</font>" onkeyup="check_amount('<?php echo $result_orderlist['product_id'] ?>');" id="change_amount<?php echo $result_orderlist['product_id'] ?>" autocomplete="off" /></td>
